...
import { ExpenseTypeDropDownRequest } from "../../APIRequest/ExpenseAPIRequest";
import { useSelector } from "react-redux";
import store from "../../redux/store/store";
import { OnChangeExpenseInput } from "../../redux/state-slice/expense-slice";
import { ErrorToast, IsEmpty } from "../../helper/FormHelper";
import { useNavigate } from "react-router-dom";
import { GetExpenseDetailRequest } from "../../APIRequest/ExpenseAPIRequest";
...
const ExpenseCreateUpdate = () => {
  ...
  useEffect(() => {
    ...
    if (id !== null) {
      (async () => {
        SetObjectID(id);
        await GetExpenseDetailRequest(id);
      })();
    }
  }, []);
  ...
  const SaveChange = async () => {
    if (IsEmpty(FormValue.TypeID)) {
      ...
      if (await ExpenseSaveRequest(FormValue, ObjectID)) {
